...
export default function SearchDashboard() {
  ...
  const fetchSearchHistory = async () => {
    ...
    try {
      const history = await apiService.getSearchHistory();
      // Add id to history items if not present (for backward compatibility)
      const historyWithIds = history.map((item: any, index: number) => ({
        ...item,
        id: item.id || index + 1,
      }));
      setSearchHistory(historyWithIds);
      setShowingHistory(true);
      setSongResults([]); // Clear song results when showing history
    } catch (error) {
      console.error('Error fetching history:', error);
  ...
  // Callback for when history is deleted
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#1a0933] text-white">
      ...
      <main className="flex-1 p-4 md:p-6 flex flex-col max-w-6xl mx-auto w-full">
        ...
        {/* Content Display - Either Search History or Results */}
        {showingHistory ? (
          <SearchHistory 
            searchHistory={searchHistory}
            loadHistoricalRecommendations={loadHistoricalRecommendations}
          ...
          <>
            ...
            {/* Audio Preview with Recommendations */}
            {songResults.length > 0 && (
              <div className="mb-6 p-4 bg-[#2d0f4c] rounded-lg">
                <h3 className="text-white font-medium mb-2">Top Tracks Preview</h3>
                
                {/* First two tracks with full player */}
                <div className="grid grid-cols-1 gap-4 mb-4">
                  {songResults.slice(0, 2).map((song, index) => (
                    song.deezer_id && (
                      <div key={index} className="w-full">
                        <p className="text-sm text-purple-300 mb-1">{song.title} - {song.artist}</p>
                        <iframe 
                          title={`Deezer Player ${index + 1}`}
                          src={`https://widget.deezer.com/widget/dark/track/${song.deezer_id}?app_id=457142&autoplay=false&radius=true&tracklist=true`}
                          width="100%" 
                          height={180}
                ...
                {/* Display mini players for other results (up to 2 more) */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  {songResults.slice(2, 4).map((song, index) => (
                    <div key={index} className="bg-[#3a1866] rounded-md p-3">
                      <div className="font-medium text-sm mb-2">{song.title} - {song.artist}</div>
                      {song.spotify_url && (
                        <a
                          href={song.spotify_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-xs bg-[#1DB954] text-white px-2 py-0.5 rounded-full flex items-center gap-1 hover:opacity-90 transition-opacity w-fit"
                        >
                          <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z" />
                          </svg>
