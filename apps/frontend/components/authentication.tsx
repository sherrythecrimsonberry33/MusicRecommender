...
export function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle the structured error response from the backend
      try {
        // Try to parse the error detail as JSON
        const errorDetail = JSON.parse((error as Error).message);
        ...
        if (errorDetail.field && errorDetail.field !== 'general') {
          newErrors[errorDetail.field as keyof typeof newErrors] = errorDetail.message;
        } else {
          newErrors.general = errorDetail.message;
        }
